...
FileHandler::FileHandler(bool useFAT) : _useFAT(useFAT) {}
...
esp_err_t FileHandler::handler(httpd_req_t* req) {
    ...
    if (_useFAT) {
      ...
      for (int i = 0; i < length; i += readSize) {
        ...
        if (ptr) {
          char temp[12] = {0};
          sprintf(temp,"v. %.5s",appVersion); // 11 chars max!
          memset((void *)ptr, ' ', sizeof(temp) - 1); // prepare replacement with spaces
          temp[11] = 0;
          memcpy((void *)ptr, (void *)temp,
                 sizeof(temp) - 1); // patch version in (no final 0)
        }
